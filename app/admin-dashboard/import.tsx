...
export default function ImportPage() {
  ...
  const handleUpload = async () => {
    ...
    setLoading(true);
    setMessage('Uploading to API server...');
    setResult(null);
    setJobProgress(null); // reset only when starting new run

    const formData = new FormData();
    formData.append('file', file);

    try {
      const start = await apiFetch<{ success?: boolean; jobId?: string; error?: string }>('/api/admin/events/import', {
        method: 'POST',
        body: formData,
        auth: true,
      });
      ...
      if (!start.success || !start.jobId) {
        setMessage(`❌ ${start.error || 'Import failed to start. Are you logged in as admin?'}`);
        return;
      }

      setMessage('Processing import on backend...');

      let last: ImportJobPayload | null = null;
      for (let i = 0; i < 600; i++) {
        const res = await apiFetch<{ success?: boolean; job?: ImportJobPayload }>(
          `/api/admin/events/import/${start.jobId}`,
          { auth: true },
        );
        if (!res.success || !res.job) break;
        last = res.job;
        setJobProgress({
          processed: res.job.processedRows,
          total: res.job.totalRows,
          status: res.job.status,
        });
        if (res.job.status === 'COMPLETED' || res.job.status === 'FAILED') break;
        await new Promise((r) => setTimeout(r, 1200));
      }

      if (!last) {
        setMessage('❌ Could not read import job status.');
        return;
      }
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8">
        ...
        {jobProgress && loading && (
          <div className="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg text-indigo-900 text-sm">
            <p className="font-medium">Server import: {jobProgress.status}</p>
            <p>
              Rows {jobProgress.processed} / {jobProgress.total}
            </p>
            <div className="mt-2 h-2 bg-indigo-100 rounded overflow-hidden">
              <div
                className="h-full bg-indigo-600 transition-all"
                style={{
                  width: `${jobProgress.total ? Math.min(100, (jobProgress.processed / jobProgress.total) * 100) : 0}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="mt-10 p-6 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div className="mt-4 p-4 bg-slate-100 border border-slate-200 rounded text-sm text-slate-700">
            <strong>Backend import:</strong> Files are parsed and events are created on the Biz API server (not in the browser).
            Format date columns as Text in Excel to avoid odd Excel serial dates. New organizers receive a thank-you email with an
            optional &quot;Set your password&quot; link when <code className="bg-slate-200 px-1 rounded">EMAIL_USER</code> /{' '}
            <code className="bg-slate-200 px-1 rounded">EMAIL_PASS</code> are set on the backend.
          </div>
        </div>
      </div>
    </div>
