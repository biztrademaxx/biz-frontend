...
export function HousekeepingForm({ eventId, organizerId }: HousekeepingFormProps) {
  ...
  const housekeepingCostPerShift = 2000 // INR per 10-hour shift

  const calculateTotal = () => {
    const subtotal = formData.numberOfDays * housekeepingCostPerShift
    ...
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="p-6">
        <Alert className="mb-6">
          ...
          <AlertDescription>
            The Organisers will be providing General Housekeeping in the exhibition hall from 20-22 November 2025 only.
            However, Housekeeping at individual stall can be arranged from 18th to 19th November 2025. These facilities
            will be recruited from the approved Agency for the Exhibition.
          </AlertDescription>
        </Alert>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-4">Housekeeping Requirements</h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <Label>Category</Label>
                <Input value="Housekeeping" disabled className="bg-gray-50" />
              </div>
              <div>
                <Label>Charges per Shift (10 Hours)</Label>
                <Input value="₹2,000" disabled className="bg-gray-50" />
              </div>
            </div>
            ...
            <div className="p-3 bg-gray-50 rounded">
              <div className="flex justify-between text-sm">
                <span>
                  Subtotal ({formData.numberOfDays} days × ₹{housekeepingCostPerShift}):
                </span>
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          <p className="text-sm text-gray-600">
            <strong>Important:</strong> Housekeeping services are only available during setup days (18th-19th November
            2025). General housekeeping will be provided by organizers during exhibition days (20th-22nd November 2025).
          </p>
        </div>
      </CardContent>
    </Card>
