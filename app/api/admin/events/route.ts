...
/** Accept string or string[] from clients; always persist a flat text[] for `eventType`. */
function normalizeEventTypeArray(raw: unknown): string[] {
  if (Array.isArray(raw)) {
    return raw.flatMap((x) => (x != null ? String(x).trim() : "")).filter(Boolean)
  }
...
async function findOrCreateUser(userData: {
  ...
  const newUser = await prisma.user.create({
    data: {
      ...
      phone: data.phone || "",
      avatar: data.avatar || "/city/c4.jpg",
...
export async function POST(request: Request) {
  try {
    ...
    const eventData: any = {
      ...
      eventType: normalizeEventTypeArray(body.eventType),
